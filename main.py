...
def get_chat_session(user_id, is_vip, mode):
    ...
    if user_id not in user_chats:
        # Стабильная действующая модель Gemini
        model_name = "gemini-2.0-flash" if is_vip else "gemini-1.5-flash"
        try:
            model = genai.GenerativeModel(model_name, system_instruction=system_instruction)
            user_chats[user_id] = model.start_chat(history=[])
        except Exception:
            # Запасной вариант при сбоях конкретной модели
...
def handle_all_messages(message):
    ...
    if text == "🧹 Очистить диалог":
        ...
        try:
            ...
            # Очищаем поврежденную сессию автоматически при ошибке
            if user_id in user_chats:
                del user_chats[user_id]
