...
# --- 4. РАБОТА С НЕЙРОСЕТЬЮ (GROQ) ---
user_histories = {}

def get_ai_response(user_id, user_message, is_vip, mode):
    system_prompt = "Тебя зовут ИИННОКЕНТИЙ. Ты вежливый, мудрый и находчивый ИИ-ассистент. Отвечай на языке пользователя."
    if mode == "coder":
    ...
    # Llama 3.3 70B — флагманская мощная модель для всех
    # Llama 3.1 8B — ультра-быстрая легкая модель
...
def send_welcome(message):
    ...
    welcome_text = (
        ...
        "Я **ИИННОКЕНТИЙ** — твой личный ИИ-помощник на базе Llama 3.3!\n"
...
def handle_all_messages(message):
    ...
    if text == "🧹 Очистить диалог":
        ...
        markup = types.InlineKeyboardMarkup()
        btn_text = types.InlineKeyboardButton("⭐️ VIP Текст (Llama 70B) - 50 Stars", callback_data="buy_text_vip")
        ...
        status_text = "⭐️ **VIP (Llama 70B)**" if user_data["text_vip"] else "👤 **Standard (Llama 8B)**"
        ...
        try:
            ...
            bot.reply_to(message, f"⚠️ Ошибка Groq API: `{str(e)}`", parse_mode="Markdown")

# --- 6. CALLBACKS И ГЕНЕРАЦИЯ КАРТИНОК ---
...
def send_stars_invoice(call):
    if call.data == "buy_text_vip":
        title = "VIP Доступ (Llama 70B)"
        description = "Улучшенная нейросеть высокой мощности."
...
if __name__ == "__main__":
    ...
    while True:
        try:
            bot.remove_webhook()
            time.sleep(1)
            print("Бот ИИннокентий на Groq запущен!")
