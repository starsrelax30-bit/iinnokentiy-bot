...
def handle_all_messages(message):
    ...
    if text == "🧹 Очистить диалог":
        ...
        markup = types.InlineKeyboardMarkup()
        btn_text = types.InlineKeyboardButton("⭐️ VIP Текст (Gemini Pro) - 50 Stars", callback_data="buy_text_vip")
        btn_img = types.InlineKeyboardButton("🎨 VIP Картинки - 100 Stars", callback_data="buy_image_vip")
        markup.add(btn_text)
        markup.add(btn_img)
        bot.send_message(message.chat.id, "Покупка подписок через **Telegram Stars** (или активируйте промокод в профиле):", reply_markup=markup)

    elif text == "ℹ️ Статус":
        status_text = "⭐️ **VIP (Gemini Pro)**" if user_data["text_vip"] else "👤 **Standard (Flash)**"
        ...
        bot.send_message(message.chat.id, f"👑 **Админ Панель ИИннокентия**\n\nВсего пользователей в базе: `{users_count}`", parse_mode="Markdown", reply_markup=markup)

    elif text == "🛠 Поддержка":
        support_text = f"🛠 **Служба поддержки**\n\n• Админ: {SUPPORT_ADMIN}\n• Канал: [Ссылка]({SUPPORT_CHANNEL})"
        ...
        if user_data["image_vip"]:
            ...
            bot.send_message(message.chat.id, "🔒 Купите VIP или введите промокод (`ART2026`) в разделе 'ℹ️ Статус'!")
        ...
        try:
            ...
            bot.reply_to(message, "⚠️ Произошла ошибка. Нажмите кнопку '🧹 Очистить диалог' и попробуйте снова.")
...
def send_stars_invoice(call):
    if call.data == "buy_text_vip":
        ...
        description = "Умная модель Gemini 1.5 Pro."
