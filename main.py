...
def get_chat_session(user_id, is_vip, mode):
    ...
    if user_id not in user_chats:
        # Используем актуальную базовую модель
...
def handle_all_messages(message):
    ...
    if text == "🧹 Очистить диалог":
        ...
        bot.send_message(message.chat.id, f"👑 **Админ Панель ИИннокентия**\n\nВсего пользователей в базе: `{users_count}`", parse_mode="Markdown", markup=markup)
        ...
        try:
            ...
            # Выводим конкретный текст ошибки, чтобы понять, в чем дело!
